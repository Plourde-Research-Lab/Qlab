...
% Copied from LSG drivers by Caleb Howington: Nov. 2015
...
classdef LabbrickLSG < deviceDrivers.lib.uWSource
    ...
    methods
        %Constructor
        function obj = LabbrickLSG()
            % load DLL
            % build library path
            path = [fileparts(mfilename('fullpath')) filesep];
            if ~libisloaded('vnx_fsynsth')
                loadlibrary([path 'vnx_fsynsth.dll'], [path 'vnx_fsynsth.h']);
                calllib('vnx_fsynsth', 'fnLSG_SetTestMode', false);
            end
        end
        ...
        function connect(obj, serial)
            ...
            status = calllib('vnx_fsynsth', 'fnLSG_InitDevice', obj.devID);
            if status ~= 0
            ...
            obj.open = 1;
            obj.max_power = calllib('vnx_fsynsth', 'fnLSG_GetMaxPwr', obj.devID) / 4;
            obj.min_power = calllib('vnx_fsynsth', 'fnLSG_GetMinPwr', obj.devID) / 4;
            obj.max_freq = calllib('vnx_fsynsth', 'fnLSG_GetMaxFreq', obj.devID) / 1e8;
            obj.min_freq = calllib('vnx_fsynsth', 'fnLSG_GetMinFreq', obj.devID) / 1e8;
        end

        function disconnect(obj)
            if obj.open
                status = calllib('vnx_fsynsth', 'fnLSG_CloseDevice', obj.devID);
                if status ~= 0
        ...
        function flag = get_dev_status(obj, status_code)
           val = calllib('vnx_fsynsth', 'fnLSG_GetDeviceStatus', obj.devID);
        ...
        % fnLSG_GetDevInfo to only return device IDs in order until it
        ...
        % call to fnLSG_GetDevInfo. To deal with this, we store the IDs and
        ...
        function [ids, serials] = enumerate(obj)
            ...
            %num_devices = calllib('vnx_fsynsth','fnLSG_GetNumDevices');
            ...
            if isempty(devIDs)
                ...
                [~, devIDs] = calllib('vnx_fsynsth', 'fnLSG_GetDevInfo', devIDs);
                for i = 1:num_devices
                    id = devIDs(i);
                    serial_nums(i) = calllib('vnx_fsynsth', 'fnLSG_GetSerialNumber', id);
                end
        ...
        function name = model_name(obj)
            [~, obj.model] = calllib('vnx_fsynsth', 'fnLSG_GetModelNameA', obj.devID, '          ');
            name = obj.model;
        end
        ...
        function val = get.frequency(obj)
            ...
            val = calllib('vnx_fsynsth', 'fnLSG_GetFrequency', obj.devID) * 10;
            % convert to GHz
            val = val/1e9;
        end
        function val = get.power(obj)
            % returns power as attenuation from the max output power, in an integer multiple of 0.25dBm.
            attenuation = calllib('vnx_fsynsth', 'fnLSG_GetPowerLevel', obj.devID) / 4;
            val = obj.max_power - attenuation;
        end

        function val = get.output(obj)
            val = calllib('vnx_fsynsth', 'fnLSG_GetRF_On', obj.devID);
        end
        ...
        function val = get.pulse(obj)
            ...
            if strcmp(obj.pulseSource, 'int')
                val = calllib('vnx_fsynsth', 'fnLSG_GetPulseMode', obj.devID);
            else
                val = obj.pulseModeEnabled;
            end
        end
        function val = get.pulseSource(obj)
            devVal = calllib('vnx_fsynsth', 'fnLSG_GetUseInternalPulseMod', obj.devID);
        ...
        function val = get.refSource(obj)
            val = calllib('vnx_fsynsth', 'fnLSG_GetUseInternalRef', obj.devID);
        ...
        function obj = set.frequency(obj, value)
            ...
            calllib('vnx_fsynsth', 'fnLSG_SetFrequency', obj.devID, value*1e8);
        end
        function obj = set.power(obj, value)
            ...
            calllib('vnx_fsynsth', 'fnLSG_SetPowerLevel', obj.devID, value);
        end
        function obj = set.output(obj, value)
            % don't know why I need them, but without the pause commands
            % the SetRFOn command is occasionally ignored
            pause(0.1);
            calllib('vnx_fsynsth', 'fnLSG_SetRFOn', obj.devID, obj.cast_boolean(value));
            pause(0.1);
        end
        ...
        function obj = set.pulse(obj, value)
            ...
            switch obj.pulseSource
                case 'int'
                    calllib('vnx_fsynsth', 'fnLSG_SetUseExternalPulseMod', obj.devID, false);
                    calllib('vnx_fsynsth', 'fnLSG_EnableInternalPulseMod', obj.devID, obj.pulseModeEnabled);
                case 'ext'
                    calllib('vnx_fsynsth', 'fnLSG_EnableInternalPulseMod', obj.devID, false);
                    calllib('vnx_fsynsth', 'fnLSG_SetUseExternalPulseMod', obj.devID, obj.pulseModeEnabled);
                otherwise
        ...
        function obj = set.refSource(obj, value)
            ...
            calllib('vnx_fsynsth', 'fnLSG_SetUseInternalRef', obj.devID, checkMapObj(lower(value)));
