...
classdef SNRFidelity < handle
    ...
    methods
        ...
        function Init(obj, settings)
            ...
            for instrument = fieldnames(instrSettings)'
                ...
                %To get a different sequence loaded into the APS1 when used as a slave for the msm't only.
                    ...
                    %else
                        instrSettings.(instrument{1}).seqFile = fullfile(getpref('qlab', 'awgDir'), 'MEAS', ['MEAS-' instrument{1} '.' ext]);
                    %end
                end
            ...
            add_measurement(obj.experiment, 'SNR',...
                MeasFilters.SNR('SNR', struct('dataSource', obj.settings.dataSource, 'plotMode', 'real/imag', 'plotScope', true)));
        ...
        function SNRData = Do(obj)
            obj.experiment.run();
%             drawnow();
            SNRData = obj.experiment.data.SNRData;
        end
    end
end
