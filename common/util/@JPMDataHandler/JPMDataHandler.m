% A wrapper around MATLAB's JPM reading/writing methods to support the
...
classdef JPMDataHandler < handle
    properties
        FID
        fileName
        fileOpen = 0
        idx % index into file
        varidx % index into file for variances
        rowSizes
        columnSizes
        dimensions
        nbrDataSets
    ...
    methods
        function obj = JPMDataHandler(fileName)
        ...
        function open(obj, headerStruct, dataSetInfo)
            ...
            % withVar - (default false) boolean to control whether to write
            ...
            obj.varidx = ones(obj.nbrDataSets, 1);
            ...
            for ct = 1:length(dataSetInfo)
                ...
                elseif dataSetInfo{ct}.dimension == 3
                    ...
                    error('JPMDataHandler does not support data sets of dimension %d', dataSetInfo{ct}.dimension);
                end
            end
        end
        ...
        function open1dDataSet(obj, setNumber, info)
            ...
            %open data set
            h5create(obj.fileName, ['/DataSet' num2str(setNumber) '/counts'], Inf, 'ChunkSize', 10);
%             h5create(obj.fileName, ['/DataSet' num2str(setNumber) '/imag'], Inf, 'ChunkSize', 10);
        ...
        function open2dDataSet(obj, setNumber, info)
            ...
            %open data set
            h5create(obj.fileName, ['/DataSet' num2str(setNumber) '/counts'], [Inf obj.rowSizes(setNumber)], 'ChunkSize', [10 obj.rowSizes(setNumber)]);
%             h5create(obj.fileName, ['/DataSet' num2str(setNumber) '/imag'], [Inf obj.rowSizes(setNumber)], 'ChunkSize', [10 obj.rowSizes(setNumber)]);
        ...
        function open3dDataSet(obj, setNumber, info)
            ...
            h5create(obj.fileName, ['/DataSet' num2str(setNumber) '/counts'], [Inf obj.columnSizes(setNumber) obj.rowSizes(setNumber)], 'ChunkSize', [10 obj.columnSizes(setNumber) obj.rowSizes(setNumber)]);
%             h5create(obj.fileName, ['/DataSet' num2str(setNumber) '/imag'], [Inf obj.columnSizes(setNumber) obj.rowSizes(setNumber)], 'ChunkSize', [10 obj.columnSizes(setNumber) obj.rowSizes(setNumber)]);
        ...
        function writeHeader(obj, headerStruct)
            ...
            obj.writeString('/header', json.dump(headerStruct, 'indent', 2));
        ...
        function write(obj, val, dataSet)
            ...
            switch obj.dimensions(dataSet)
                ...
                case 2
                    ...
                    elseif nsdims(val) == 1 % 1D row/column
                        obj.writeRow(val, dataSet);
        ...
        function writePoint(obj, val, dataSet)
            ...
            h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/counts'], val, obj.idx(dataSet), 1);
%             h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/imag'], imag(val), obj.idx(dataSet), 1);
        ...
        function writeRow(obj, row, dataSet)
            ...
            switch obj.dimensions(dataSet)
                case 1
                    h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/counts'], row, 1, length(row));
%                     h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/imag'], imag(row), 1, length(row));
                case 2
                    h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/counts'], row, [obj.idx(dataSet) 1], [1 obj.rowSizes(dataSet)]);
%                     h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/imag'], imag(row), [obj.idx(dataSet) 1], [1 obj.rowSizes(dataSet)]);
        ...
        function writePage(obj, page, dataSet)
            ...
            switch obj.dimensions(dataSet)
                ...
                case 2
                    h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/counts'], page, [1 1], size(page));
%                     h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/imag'], imag(page), [1 1], size(page));
                case 3
                    h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/counts'], page, [obj.idx(dataSet) 1 1], [1 obj.columnSizes(dataSet) obj.rowSizes(dataSet)]);
%                     h5write(obj.fileName, ['/DataSet' num2str(dataSet) '/imag'], imag(page), [obj.idx(dataSet) 1 1], [1 obj.columnSizes(dataSet) obj.rowSizes(dataSet)]);
    ...
    methods (Static)
        function out = UnitTest()
            out = JPMDataHandler.UnitTest1d(0) && JPMDataHandler.UnitTest2d(0) && JPMDataHandler.UnitTestBufferd2d(0) && JPMDataHandler.UnitTestMultiBufferd2d(0) && JPMDataHandler.UnitTest3d(0);
        end
        ...
        function out = UnitTest1d(verbose)
            ...
            dataHandler = JPMDataHandler('unit_test.h5');
